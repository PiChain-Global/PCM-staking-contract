...
abstract contract PCMStakeBase is Ownable {
    ...
    function addBonusFunds(uint256 _amount) public {
        uint256 _blanceBefore = token.balanceOf(address(this));
        token.safeTransferFrom(_msgSender(), address(this), _amount);
        uint256 _blanceAfter = token.balanceOf(address(this));
        _funds += _blanceAfter - _blanceBefore;
    ...
    function userDeposit(address _user, uint256 _amount) internal {
        ...
        uint256 _blanceAfter = token.balanceOf(address(this));
        require(_blanceAfter - _blanceBefore >= _amount, "PCM::DepositFailed");
...
contract PCMStake is PCMStakeBase {
    ...
    uint256 constant private MONTH_3 = 90;
    uint256 constant private MONTH_6 = 180;
    uint256 constant private YEAR_1 = 365;
    uint256 constant private YEAR_2 = 365 * 2;
    uint256 constant private YEAR_3 = 365 * 3;
    ...
    */
    function getAprAndLimit(StakeType _type) public view returns (uint256,uint256) {
        (uint256 _year, uint256 _month) = getYearsMonth();
        StakeReward storage _stakeData = stakeData[_type];
    ...
    function _calculatedReward(StakeType _type, uint256 _amount, uint256 _apr) internal pure returns (uint256) {
        if (_type == StakeType.s90Days) {
            return _amount * _apr / BASE * MONTH_3 / YEAR_1;
        } else if (_type == StakeType.s180Days) {
            return _amount * _apr / BASE * MONTH_6 / YEAR_1;
        } else if (_type == StakeType.s1Year) {
            return _amount * _apr / BASE * YEAR_1 / YEAR_1;
        } else if (_type == StakeType.s2Year) {
            return _amount * _apr / BASE * YEAR_2 / YEAR_1;
        } else if (_type == StakeType.s3Year) {
            return _amount * _apr / BASE * YEAR_3 / YEAR_1;
        }
        return 0;
    }
}